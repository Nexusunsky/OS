...
'''
      ...
      8，管程：
      尽管信号量 方便 有效，处理进程同步，但是使用不正确 容易出错，并且难以检查，排错。
        回顾：
            使用 信号量 解决临界区的问题，所有进程共享一个信号变量mutex，其初始化为1，
            每个进程在 进入临界区之前 执行wait(mutex)，之后执行 signal(mutex)。

        问题：
            使用信号量时 没有遵守约定 使得时序不正确 便容易造成 互斥的破坏 或者 死锁的发生。

        方案：
            为了处理这些问题，产生了 一些 高级语言构造，本节介绍 一种基本，高级的同步构造，管程 类型。

        管程（monitor）类型 提供了 一组在 管程内 互斥 的操作，
        管程类型 的表示，包括了：一组变量的声明（这些变量的值定义了 一个类型实例的状态 ）和对这些 变量操作的子程序 和函数 的实现。
        管程类型 的表示 不能 直接为 各个进程 所使用，
            因此，管程内定义的 子程序 只能 访问位于管程内那些 局部变量的声明 和 形式参数，
            类似地，管程局部变量只能被局部子程序访问。

        管程结构 确保 一次 只能 有一个进程 在管程内活动。
        但是需要 额外的同步机制，可用 条件(condition)结构来提供。
            condition x, y;
            对条件变量仅有的操作是wait()和signal()，
...
'''
        考虑管程 ResourceAllocator 用于在 多个进程 控制对 单个资源 的访问，每个进程在请求资源分配时，说明它计划使用资源的最大时间，
        管程将资源非配给具有最短分配请求的进程。而 访问有关资源的进程必须按照如下顺序进行。
        //R 是 ResourceAllocator
        R.acquire();
            ...
            access the resource
            ...
        R.release();
        而管程并不能保证 以上顺序的正确执行，并可能引起些问题：
            a，一个进程没有获得资源访问权限就访问资源
            b，一个进程在获得资源访问权限后就不释放权限
            c，一个进程可能试图释放一个从来没有请求的权限
            d，一个进程请求同一资源两次

        而确保 进程遵守适当的顺序，必须检查所有 管程 ResourceAllocator 和其他管理资源的程序。
        为了确保系统的正确性，有两个条件是必须检查：
            1，用户进程必须总是按照正确顺序调用管程
            2，必须确保一个不合作进程不能简单忽略由管程所提供的互斥关口，以及在不遵守协议的情况下访问共享资源
            只有确保这两个条件，才能保证没有时间依赖性错误发生，且调度算法不会失败。


        案例
            JAVA 管程：

        Java为线程同步提供一个类似与 管程的并行机制，Java中的 每个对象 都有一个单独的锁(monitor)。
        '''
        public class Simple {
        ...
        ...
        Simple s = new Simple();
        s.safeMethod();
        当调用时，需要获取 对象s 的锁，如果锁 被其他线程 占用，则调用同步方法的 线程 阻塞，并被放在 对象锁的进入集合中。
        进入集合 表示 正在等待 所可用的线程集合。
        如果 同步方法被调用 且 锁可用，调用线程 将成为 对象锁的拥有者，便可以直接进入该方法
        当线程退出时，释放锁，同时从进入集合中选择一个线程成为锁的新的 拥有者。
        ...
        同步案例研究：
            1，Solaris同步：
                ...
                适应互斥（adaptive mutex）：保护对每个临街数据项的访问。
                    再多处理器系统中，适应互斥 以 自旋锁 实现的标准信号量 开始。
                    如果 数据已加锁，那么适应互斥 由两个选择
                        自旋锁：   a，如果锁 被正在另一个CPU上运行的线程所拥有，那么 拥有锁 的线程可能会 很快结束，那么等待锁的 线程自旋 并等待锁可用。
                        阻塞（睡眠）b，如果拥有锁的线程 不处于运行状态，那么线程就阻塞进入睡眠，直到 锁释放时 被唤醒，因为这类锁不能很快释放。睡眠比自旋要更合适。
                    适应互斥：
                        保护 那些为 较短代码段 所访问的数据，
                        如果 代码较长，自旋等待就极为 低效了，对于较长代码段，可以使用 条件变量 和 信号量。

                    过程描述：
                        如果所要的锁被占，那么线程等待进入睡眠
                        当一个 线程 释放锁，它发出一个 信号 给队列中下一个 睡眠线程。
                        性能比较：线程 进入睡眠和被唤醒以及相关上下文的切换所带来的开销，比在 自旋锁上浪费上百条指令的 开销少。

                十字转门（turnstile）是一个 队列结构，包含 阻塞在锁上 的线程。
                    a，如果 一个线程 现拥有 同步对象的锁，那么所有 其他线程 试图 获取锁时会 阻塞 并进入该锁的 十字转门。
                    b，当释放该锁时，内核 会从 十字转门 中选择一个线程以作为 锁的下一个拥有者。

                Solaris 使用 十字转门 以安排 等待获取 适应互斥 和 读写锁 的线程链表，
                Solaris不是将 每个同步对象 与一个 十字转门 相关联，而是给每个内核线程一个十字转门，
                注意：内核使用的 锁机制 与 用户线程 所用的锁 都是 使用同样的实现，因此 同样类型的锁 在 内核 内外都可用。

            2，Windows XP
                在多处理器系统上，Windows XP 只使用 自旋锁 来保护对 全局资源的访问。
                同Solaris一样，内核只使用自旋锁来保护 较短代码段，由于效率考虑，内核会确保拥有 自旋锁的线程 觉不会被抢占。

                对于内核外线程的 同步，Windows XP 提供了 调度对象：
                采用调度对象，线程可根据 多种不同的机制，包括互斥，信号量，事件和定时器，进行同步。
                共享数据 可以通过要求 线程 获取访问数据的 互斥拥有权 和使用完后 释放拥有权 加以保护。
                事件是一个 同步机制，其使用与条件变量相似
                    调度对象 拥有来个状态，触发状态 和 非触发状态。
                        触发状态：表示 对象可用 且线程在获取它时 不会阻塞。
                        非触发状态：表示 对象不可用 且当线程试图获取它时 会阻塞。


            3，Linux
                Linux 2.6版本以前 为非抢占内核，即纵然有更高优先级的进程可以运行，也不能抢占内核模式下运行的其他进程。
                然而，现在的 linux的内核 完全可以抢占，这样在 内核状态 下的任务也能 被抢占。
                Linux 内核提供了 自旋锁 和 信号量 （还有这两种版本的 读写锁）以进行 内核加锁。
                        对于SMP (对称多处理)机器，基本的 锁机制为自旋锁，短期运行 可采用 自旋锁。
                        对于单处理器 机器，自旋锁不适合使用，可以替换成 内核抢占 的 允许与禁止。
                            系统调用：preempt_disable()  preempt_enable()，以用于禁止 与 允许内核抢占。

                        自旋锁 和 内核抢占允许与禁止 一起 都只是 短期占有锁 时 可以用于内核
                        当 一个 锁需要长时间占有锁时，使用 信号量 更合适。

            4，Pthread同步
                Pthread API 为线程同步 ，提供 互斥锁，条件变量，读写锁。
