# CPU 调度
    ...
    9，多处理器调度
        讨论多处理器功能相同（同构）系统，可以将 任何处理器 用于 运行 队列内的任何进程。

        a，多处理器调度方法：
            在多处理器中，CPU调度
                一种方法是
                    让一个 处理器（主服务器）处理所有的 调度决定，IO处理以及其他系统活动，其他的处理器只执行代码。这种非对称多处理方法更为简单，因为只有一个处理器访问系统数据结构，减轻了数据共享的需要。
                另一种方法
                    是使用 对称多处理，（symmetric multiprocessing SMP），即每个处理器自我调度。
                    所有进程 可能处于 一个共同的就绪队列中，或每个 处理器 都有它自己的 私有就绪进程队列。
                无论如何，调度通过 每个处理器 检查 共同就绪队列 并选择一个进程来执行。
                如果 多个处理器 试图访问和更新一个 共同数据结构，那么 每个处理器 必须仔细编程：
                    必须确保 两个处理器 不能选择同一进程，且进程不会从队列中丢失。

        b，处理器亲和性
            进程最近访问的数据进入处理器缓存，那么进程所进行的联系内存访问通常可以通过缓存得到。
            而当进程移动到其他处理器上时，被迁移的第一个处理器的缓存中的内容必须为无效，而将要迁移到的第二个处理器的缓存需要重新构建，
            而无论是缓存的迁移和重建 的代价都很高，绝大多数SMP系统试图避免将进程从一个处理器移至另一个处理器上运行。
            这被称为 处理器亲和性：即一个进程需要一种对其运行所在的处理器的亲和性。
                处理器亲和性 有几种形式：
                    操作系统 具有设法让一个 进程 保持在同一个 处理器上 运行的策略，但是这不能作出任何保证称为 软亲和性。
                    有些系统提供了 硬亲和性的系统调用，从而允许进程指定它不允许移至其他处理器上，
                    
        
        c，负载平衡（load balancing）
            在SMP系统中。保持 所有处理器 工作负载平衡，以完全利用多处理器的优点。
            负载平衡设法 将工作负载均衡 分配到 SMP系统 中的 所有处理器上。 
            负载均衡通常只是对那些 拥有自己私有的 可执行进程的处理器 而言是必要的。
            在具有共同队列的系统中，通常不需要 负载均衡，因为一旦 处理器空闲 它便会从 共同队列 中取走一个 可执行进程。
            在支持SMP的当代操作系统中，每个处理器 都具有一个 可执行进程的私有队列。
            负载平衡 通常有两种方式：
                push migration：
                    一个特定的任务 周期性地检查 每个处理器的负载，如果发现不平衡，即通过将 进程 从超载处理器移到（或推送）空闲或不太忙的处理器，从而平均地分配负载。
                和
                pull migration：
                    当空闲处理器从一个忙的处理器上推送（pull）一个等待任务时，发生pull migration

            push migration 和 pull migration不能相互排斥，事实上，在负载均衡系统中它们常常被并行地实现。

        负载均衡会 抵消 处理器的亲和性。

    10，对称多线程
        通过提供多个 物理处理器，SMP系统 允许同时 允许几个线程。
        另一方法是提供多个 逻辑处理器 实现。这种方法被称为 对称多线程（SMT），在Intel处理器中，它也被称为超线程（hyperthreading）技术。
        SMT的思想是在同一个 物理处理器 上 生成 多个逻辑处理器，向操作系统呈现一个 多逻辑处理器的视图，即使 系统仅有单处理器。每个 逻辑处理器 都有它自己的 架构状态。
        SMT是 硬件 而不是 软件 提供的，硬件提供每个 逻辑处理器的架构状态 表示以及 中断处理方法。
        操作系统不必被特殊设计，但是如果能意识到 所允许的系统的实现，可以在一点程度上提升性能。

    11，线程调度
        操作系统调度 的是 内核线程，而不是进程。
        用户线程由线程库管理，内核并不了解它们。
        为了能在CPU上运行，用户线程必须最终映射到相应的内核级线程，尽管这种映射是是间接的，可能使用轻量级进程（LWP）。

        a，竞争范围
            用户线程和内核线程的区别之一在于它们是如何被调度的。
            进程竞争范围（process-contention scope，PCS）方法：
                执行 多对一模型 和 多对多模型 的系统上，线程库调度 用户级别线程到一个有效的LWP上运行。
            CPU竞争 发生 同一个进程内 的 线程之间。

            提及线程库 调度用户线程到有效的LWP 上时，并不意味着用户线程就在CPU上运行，而这需要 操作系统 将 内核线程 调度到 物理CPU上。
            系统竞争范围（system-contention scope，SCS）决定 调度 哪个 内核线程 到CPU。
    ...
    12，算法评估
        如何选择CPU调度算法用于特定系统？
        定义用于选择算法的准则：而准则通常是 通过使用率 来定义的，通过 CPU使用率，响应时间，或吞吐量等。
        一种主要类型的评估方法是分析评估法（analytic evaluation）：使用给定算法和公式产生一个公式或数字，以评估对于该负荷算法的性能。

        a，确定模型：
            采用特殊预先确定的负荷，计算在给定负荷下每个算法的性能。
            特点：简单，快速。
        b，排队模型：
            许多系统上运行的 进程 每天都在变化，因此 没有静态的进程（或时间）集合用于 确定模型。
            计算机系统可以描述为服务器网络。
            每个 服务器 都有一个 等待进程队列，
                CPU是具有就绪队列的服务器，
                IO是系统是具有设备队列的服务器
        c，模拟：
            模拟涉及对计算机系统进行 建模，
            模拟程序 有一个变量 以表示时钟：当该 变量的值增加 时，模拟程序 会修改 系统状态 以反映 设备，进程和调度程序 的活动。
            跟踪磁带：
                通过监视真实的系统，纪录时间发生的顺序来建立跟踪磁带，然后使用这个顺序来驱动模拟
            模拟常常需要数小时的计算时间，为了提供更精确的结果，需要更细致的模拟，但是这些都需要大量的时间，而磁带跟踪需要大量的存储空间。
            最后，模拟程序的设计，编码，调试都是其主要的工作。

        d，实现：
            即使是模拟，精确度也是有限的，针对评估调度算法，唯一完全精确的方法是 将其放到操作系统中编程，并观察它的工作 。
            主要问题是代价太大。
