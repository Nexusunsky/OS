...
六，进程间通信
    ...
    消息传递系统：
        ...
        A，直接通信：通信的每个进程必须明确地命名通信的接受者或发送者。
        ...
        2，同步
            进程间的通信的原语的实现可以有不同的设计选项。消息传递可以是阻塞型或非阻塞型----也称为同步或异步
            集合点：当send()和receive()都阻塞时，则在接受者和发送者直接就有一个集合点。



        3，缓冲
            不管通信是直接还是间接的，通信进程所交换的消息都驻留在临时队列中。
            临时队列有三种实现：
                a，零容量：队列长度为0，线路中不能有任何消息处于等待。
                b，有限容量：队列长度为有限n，因此，最多只能有n个消息驻留其中。
                    如果在发送新消息时队列未满，那么该消息可以放在队列中（或者复制消息或者保存消息的指针），
                    且发送者可以继续执行而不必等待。不过，线路容量有限。如果线路满，必须阻塞发送者直到队列中的空间可用为止。
                c，无效容量：队列长度无限，因此，不管多少消息都可以在其中等待，从不阻塞发送者。
